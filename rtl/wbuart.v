...
//
// BoxLambda changes by epsilon537.
//
////////////////////////////////////////////////////////////////////////////////
// }}}
...
module wbuart #(
    ...
    input  wire    i_cts_n,
    output  reg    o_rts_n,
    output wire    o_uart_int //BoxLambda - only one interrupt line
    // }}}
);
  ...
  localparam [2:0]
        ...
        UART_RXREG = 3'b010,
        UART_TXREG = 3'b011,
        UART_ISR   = 3'b100, //BoxLambda - Added ISR and IEN registers.
  UART_IEN = 3'b101;
  ...
  always @(posedge i_clk)
    if ((rx_uart_reset) || (rx_break)) begin
      ...
      r_rx_perr <= 1'b0;
      r_rx_ferr <= 1'b0;
    end else if ((i_wb_stb) && (i_wb_addr == UART_RXREG) && (i_wb_we)) begin
  ...
  // implementation to the RX FIFO (they're both UFIFOs), but the TX
  ...
  always @(posedge i_clk) begin
    if (i_reset) begin
      ...
      //Set ISR when going from empty to non-empty RX FIFO.
      if (rxf_status[0] && !rx_data_avl_status_r) rx_data_avl_isr <= 1'b1;
      rx_data_avl_status_r <= rxf_status[0];

      //Set ISR when RX FIFO reaches half-full mark.
      if (rxf_status[1] && !rx_fifo_half_full_status_r) rx_fifo_half_full_isr <= 1'b1;
      rx_fifo_half_full_status_r <= rxf_status[1];

      //Set ISR when TX FIFO reaches half-empty mark.
      if (txf_status[1] && !tx_fifo_half_empty_status_r) tx_fifo_half_empty_isr <= 1'b1;
      tx_fifo_half_empty_status_r <= txf_status[1];

      //Set ISR when TX FIFO goes from non-empty to empty.
      if (!tx_empty_n && !tx_fifo_empty_status_r) tx_fifo_empty_isr <= 1'b1;
      tx_fifo_empty_status_r <= ~tx_empty_n;

      //Acknowledge/Clear ISR bits via WB by writing 1's to ISR register.
      if ((i_wb_stb) && (i_wb_addr == UART_ISR) && (i_wb_we) && i_wb_sel[1])
        {tx_fifo_half_empty_isr, tx_fifo_empty_isr, rx_fifo_half_full_isr, rx_data_avl_isr} <=
          {tx_fifo_half_empty_isr, tx_fifo_empty_isr, rx_fifo_half_full_isr, rx_data_avl_isr} & ~i_wb_data[3:0];

      //Configure IEN register.
  ...
  //Raise interrupt line when enabled interrupt sources are active.
  ...
  //WB return data for ISR read
  assign wb_isr_data = {
    28'd0, tx_fifo_half_empty_isr, tx_fifo_empty_isr, rx_fifo_half_full_isr, rx_data_avl_isr
  };

  //WB return data for IEN read
  assign wb_ien_data = {
    28'd0, tx_fifo_half_empty_ien, tx_fifo_empty_ien, rx_fifo_half_full_ien, rx_data_avl_ien
  };
